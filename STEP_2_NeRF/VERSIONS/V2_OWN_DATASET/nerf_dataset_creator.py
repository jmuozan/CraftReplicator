...
def create_nerf_dataset(reconstruction, image_dir, mask_dir, output_path, image_scale=1.0):
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        # Load the corresponding mask with different naming pattern
        if use_masks:
            # Extract the number from the image filename (assuming format like "0.jpg")
            try:
                image_number = int(os.path.splitext(image_name)[0])
                # Construct the mask filename with pattern "frame_XXXX_obj_1.png"
                mask_filename = f"frame_{image_number:04d}_obj_1.png"
                mask_path = os.path.join(mask_dir, mask_filename)
                
                # Check if mask exists with this naming pattern
                if os.path.exists(mask_path):
                    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
                    
                    # Resize if needed
                    if image_scale != 1.0:
                        mask = cv2.resize(mask, (w_target, h_target))
                    
                    # Normalize to [0, 1] and reshape to add channel dimension
                    mask_normalized = (mask.astype(np.float32) / 255.0).reshape(h_target, w_target, 1)
                    masks_array[idx] = mask_normalized
                    print(f"Loaded mask {mask_filename} for image {image_name}")
                else:
                    # If no matching mask found, create an all-ones mask (no masking)
                    print(f"Warning: No mask found for {image_name} (tried {mask_filename}). Using default all-ones mask.")
                    masks_array[idx] = np.ones((h_target, w_target, 1), dtype=np.float32)
            except ValueError:
                # If image filename doesn't follow expected pattern
                print(f"Warning: Could not determine mask name for {image_name}. Using default all-ones mask.")
                masks_array[idx] = np.ones((h_target, w_target, 1), dtype=np.float32)
