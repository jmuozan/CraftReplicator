...
def test(hn, hf, dataset, img_index, chunk_size=20, nb_bins=192, H=400, W=400):
    ...
    for i in range(int(np.ceil(H / chunk_size))):   # iterate over chunks
        ray_origins_ = ray_origins[i * W * chunk_size: (i + 1) * W * chunk_size].to(device).float()
        ray_directions_ = ray_directions[i * W * chunk_size: (i + 1) * W * chunk_size].to(device).float()
...
class NGP(torch.nn.Module):
    ...
    def forward(self, x, d):
        ...
        color = torch.zeros((x.shape[0], 3), device=x.device).float()
        log_sigma = torch.zeros((x.shape[0]), device=x.device).float() - 100000
...
def train(nerf_model, optimizer, data_loader, device='mps', hn=0, hf=1, nb_epochs=1, #10 epochs
          nb_bins=192, H=400, W=400):
    for _ in range(nb_epochs):
        for batch in tqdm(data_loader):
            ray_origins = batch[:, :3].to(device).float()
            ray_directions = batch[:, 3:6].to(device).float()
            gt_px_values = batch[:, 6:].to(device).float()
